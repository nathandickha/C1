...
function createSpaChannelWaterMesh(geometry) {
  ...
  water.userData.setWaterTuning?.({
    thicknessShallow: 0.3,
    thicknessDeep: 0.3,
    alphaShallow: 0.22,
    alphaDeep: 0.55,
    dispScale: 0.02,
...
function addChannelWaterPlane(group, sizeX, sizeY, localX, localY, center, axisX, axisY, quat, zWorld) {
  ...
  const geo = new THREE.PlaneGeometry(sizeX, sizeY, 24, 24);
...
function addCircularChannelWaterSurface(group, shape, center, quat, zWorld) {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      if (outside && arcStart === null) {
        ...
        if (arcEnd - arcStart > 0.01) {
          ...
          addCircularChannelWaterSurface(waterGroup, floorBand, center, quat, waterLevelZ);
        }
        arcStart = null;
      }
    }
