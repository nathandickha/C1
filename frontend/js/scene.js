...
const SPA_THROAT_DEBUG_BLUE_DEPTH = 0.15;
...
function updateSpaVoidDebug(scene, ground, poolGroup, spaGroup, holePts = null) {
  ...
  if (clipVolumes) {
    ...
    addDebugBox(
      group,
      Math.max(0.01, yellowClipBox.maxX - yellowClipBox.minX),
      Math.max(0.01, yellowClipBox.maxY - yellowClipBox.minY),
    ...
    addDebugBox(
      group,
      Math.max(0.01, blueClipBox.maxX - blueClipBox.minX),
      Math.max(0.01, blueClipBox.maxY - blueClipBox.minY),
...
function addChannelBox(group, sizeX, sizeY, sizeZ, localX, localY, center, axisX, axisY, quat, material, zCenter) {
  if (!(sizeX > 1e-4 && sizeY > 1e-4 && sizeZ > 1e-4)) return;
  const mesh = new THREE.Mesh(new THREE.BoxGeometry(sizeX, sizeY, sizeZ), material.clone());
...
function addCircularChannelExtrude(group, shape, depth, localZ, center, quat, material) {
  if (!group || !shape || !(depth > 1e-4)) return;
  const geo = new THREE.ExtrudeGeometry(shape, { depth, bevelEnabled: false, steps: 1, curveSegments: 48 }); geo.computeVertexNormals();
  const mesh = new THREE.Mesh(geo, material.clone()); mesh.position.set(center.x, center.y, localZ); mesh.quaternion.copy(quat); mesh.castShadow = true; mesh.receiveShadow = true; mesh.userData.isSpaChannel = true; group.add(mesh);
}
...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  if (!spaGroup.userData?.isHalfwayInWall) {
    clearSpaChannelMeshes(ground);
    return;
  }
  ...
  if (spaGroup?.userData?.spaShape === 'circular') {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      if (outside && arcStart === null) {
        ...
        if (arcEnd - arcStart > 0.01) {
          ...
          addCircularChannelExtrude(group, floorBand, channelFloorThickness, channelFloorTopZ - channelFloorThickness, center, quat, mat);
          addCircularChannelExtrude(group, wallBand, wallHeight, channelFloorTopZ, center, quat, mat);
          addCircularChannelExtrude(group, copingBand, copingDepth, copingUnderZ, center, quat, copingMat);
        }
        arcStart = null;
      }
    }
    ...
    const addBridge = (sx, sy, lx, ly) => {
      addChannelBox(group, sx, sy, channelFloorThickness, lx, ly, center, axisX, axisY, quat, mat, floorZCenter);
      addChannelBox(group, sx, sy, wallHeight, lx, ly, center, axisX, axisY, quat, mat, wallZCenter);
      addChannelBox(group, sx, sy, copingDepth, lx, ly, center, axisX, axisY, quat, copingMat, copingZCenter);
    };
...
function getSpaClipVolumes(poolGroup, spaGroup, pad = 0.01) {
  ...
  const yellowClipBox = shrinkThroatBoxAlongWall(throat, spaGroup, SPA_THROAT_YELLOW_LENGTH_SHRINK) || throat;
  const blueClipBoxBase = shrinkThroatBoxAlongWall(throat, spaGroup, SPA_THROAT_BLUE_LENGTH_SHRINK) || throat;
  const blueClipBox = extendThroatBoxAlongWall(blueClipBoxBase, spaGroup, SPA_THROAT_BLUE_LENGTH_EXTRA) || blueClipBoxBase;
