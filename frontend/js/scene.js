...
function getSpaChannelMargins(spaGroup, poolGroup = null) {
  ...
  const halfX = Math.max(0.005, (spaGroup.userData?.spaLength || 0.01) );
  const halfY = Math.max(0.005, (spaGroup.userData?.spaWidth || 0.01) );
...
function shrinkThroatBoxAlongWall(throat, spaGroup, shrink = 0) {
  ...
  if (snapSide === 'left' || snapSide === 'right') {
    const maxShrink = Math.max(0, (out.maxY - out.minY - 0.01) );
    const s = Math.min(amount, maxShrink);
    out.minY += s;
    out.maxY -= s;
  } else if (snapSide === 'front' || snapSide === 'back') {
    const maxShrink = Math.max(0, (out.maxX - out.minX - 0.01) );
    const s = Math.min(amount, maxShrink);
    out.minX += s;
    out.maxX -= s;
  }
...
function getSpaInteriorClearBox(spaGroup) {
  ...
  if (spaGroup?.userData?.spaShape === 'circular') {
    const center = new THREE.Vector3();
    spaGroup.getWorldPosition(center);
    const baseRadius = Math.max(0.01, (spaGroup.userData?.spaLength || spaGroup.userData?.spaWidth || 0.01) );
...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  if (spaGroup?.userData?.spaShape === 'circular') {
    const radius = Math.max(0.05, (spaGroup.userData?.spaLength || 0.1) );
    ...
    for (let i = 0; i <= segments; i++) {
      const a0 = i * step;
      const outside = i < segments ? isOutsideAtAngle(a0 + step ) : false;
    ...
    if (snapVariant === 'inner-wall-align') {
      ...
      if (snapSide === 'right' && margins.minX > 0) addBridge(Math.max(0.01, margins.minX), bridgeWidth, -(radius + margins.minX ), 0);
      if (snapSide === 'front' && margins.maxY > 0) addBridge(bridgeWidth, Math.max(0.01, margins.maxY), 0, radius + margins.maxY );
      if (snapSide === 'back' && margins.minY > 0) addBridge(bridgeWidth, Math.max(0.01, margins.minY), 0, -(radius + margins.minY ));
    }
...
function updateGroundMaterialSpaClip(ground, spaGroup = null, poolGroup = null) {
  ...
  const halfX = Math.max(0.01, (expanded.maxX - expanded.minX) );
  const halfY = Math.max(0.01, (expanded.maxY - expanded.minY) );
  ...
  if (uniforms?.spaRadius) {
    if (orangeOnlyVoidMode) {
      ...
      uniforms.spaRadius.value = Math.max(
        0.0,
        Math.min(r, Math.min(spaSizeWorld.x, spaSizeWorld.y) )
      );
    }
  }
