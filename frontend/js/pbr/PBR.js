...
import { updateGroundVoid, updatePoolWaterVoid } from "../scene.js";
...
export class PBRManager {
  ...
  async applyCurrentToGroup(group = null) {
    ...
    const activeSpa = spas[0] || null;
    const scene = this.poolGroup?.parent || null;
    const ground = scene?.userData?.ground || null;
    if (ground) {
      try { updateGroundVoid(ground, this.poolGroup, activeSpa); } catch (_) {}
      try { updatePoolWaterVoid(this.poolGroup, activeSpa); } catch (_) {}
