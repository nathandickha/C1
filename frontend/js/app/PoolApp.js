...
export class PoolApp {
  ...
  focusCameraOnPoolShape() {
    ...
    const halfVFov = THREE.MathUtils.degToRad(this.camera.fov );
    const halfHFov = Math.atan(Math.tan(halfVFov) * this.camera.aspect);

    const fitX = (size.x ) / Math.max(Math.tan(halfHFov), 0.01);
    const fitY = (size.y ) / Math.max(Math.tan(halfVFov), 0.01);
  ...
  focusCameraOnWall(wall) {
    ...
    const fitByWidth = (wallSpan ) / Math.max(Math.tan(halfHFov), 0.01);
    const fitByHeight = (Math.max(wallSize.z, 1.2) ) / Math.max(Math.tan(halfVFov), 0.01);
