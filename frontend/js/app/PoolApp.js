...
export class PoolApp {
    ...
    this.stepDrag = { active: false, pointerId: null, step: null, raf: 0, pendingEvent: null };
...
// --------------------------------------------------------------
  ...
  _getStepMeshes() {
    const steps = [];
    this.poolGroup?.traverse((o) => o.userData?.isStep && steps.push(o));
    steps.sort((a, b) => (a.userData?.stepIndex ?? 0) - (b.userData?.stepIndex ?? 0));
    return steps;
  }

  _getStepPlacementEdges() {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const tangent = new THREE.Vector2(dx / len, dy / len);
      const inward = ccw
        ? new THREE.Vector2(-tangent.y, tangent.x)
        : new THREE.Vector2(tangent.y, -tangent.x);
      edges.push({ index: i, a: a.clone(), b: b.clone(), len, tangent, inward });
    }
    return edges;
  }

  _getDefaultStepLayoutOffset(layout) {
    if (layout === "left") return -1;
    if (layout === "right") return 1;
    return 0;
  }

  _getStepLayoutSpan(layout, edgeLen) {
    const margin = 0.12;
    const usable = Math.max(0.6, edgeLen - margin * 2);
    if (layout === "full") return usable;
    const target = Math.min(usable, Math.max(1.2, edgeLen * 0.45));
    return Math.max(0.8, target);
  }

  applyStepLayoutFromParams() {
    if (!this.poolGroup) return;
    const steps = this._getStepMeshes();
    if (!steps.length) return;

    const edges = this._getStepPlacementEdges();
    if (!edges.length) return;

    const layout = this.poolParams.stepLayout || "full";
    const edgeIndex = Number.isInteger(this.poolParams.stepEdgeIndex) ? this.poolParams.stepEdgeIndex : 0;
    const edge = edges.find((e) => e.index === edgeIndex) || edges[0];
    if (!edge) return;

    let offsetNorm = Number(this.poolParams.stepOffset);
    if (!Number.isFinite(offsetNorm)) offsetNorm = this._getDefaultStepLayoutOffset(layout);
    offsetNorm = THREE.MathUtils.clamp(offsetNorm, -1, 1);

    const span = this._getStepLayoutSpan(layout, edge.len);
    const available = Math.max(0, edge.len - span - 0.24);
    const centerShift = available > 1e-4 ? (available * 0.5 * offsetNorm) : 0;
    const edgeMid = edge.a.clone().add(edge.b).multiplyScalar(0.5);
    const bayCenter = edgeMid.add(edge.tangent.clone().multiplyScalar(centerShift));

    const inward3 = new THREE.Vector3(edge.inward.x, edge.inward.y, 0);
    const tangent3 = new THREE.Vector3(edge.tangent.x, edge.tangent.y, 0);
    const up3 = new THREE.Vector3(0, 0, 1);
    const rotMat = new THREE.Matrix4().makeBasis(inward3, tangent3, up3);
    ...
    steps.forEach((step) => {
      ...
      const currentRun = baseLen * (step.scale.x || 1);
      const stepIndex = step.userData?.stepIndex ?? 0;
      const runCenter = currentRun * 0.5 + stepIndex * currentRun;
      const localZ = step.position.z;

      step.userData.baseStepWidth = step.userData.baseStepWidth || baseWidth;
      step.userData.baseStepLength = step.userData.baseStepLength || baseLen;

      const widthScale = span / Math.max(0.001, step.userData.baseStepWidth);
      step.scale.y = widthScale;
      step.quaternion.copy(quat);

      const pos = new THREE.Vector3(
        bayCenter.x + edge.inward.x * runCenter,
        bayCenter.y + edge.inward.y * runCenter,
        localZ
      );
  ...
  _pickStepAtPointer(event) {
    if (!this.poolGroup || !this.renderer || !this.camera) return null;
  ...
  _updateStepPlacementFromPointer(event) {
    const point = this._projectPointerToGround(event);
    if (!point) return;
    const edges = this._getStepPlacementEdges();
    if (!edges.length) return;
    ...
    for (const edge of edges) {
      const ab = new THREE.Vector2().subVectors(edge.b, edge.a);
      const ap = new THREE.Vector2(point.x - edge.a.x, point.y - edge.a.y);
      const abLenSq = Math.max(1e-6, ab.lengthSq());
      const t = THREE.MathUtils.clamp(ap.dot(ab) / abLenSq, 0, 1);
      const proj = edge.a.clone().add(ab.multiplyScalar(t));
    ...
    const layout = this.poolParams.stepLayout || "full";
    const span = this._getStepLayoutSpan(layout, best.edge.len);
    const available = Math.max(0, best.edge.len - span - 0.24);
    let offsetNorm = 0;
    if (available > 1e-4) {
      const centerDist = (best.t * best.edge.len) - (best.edge.len * 0.5);
      offsetNorm = THREE.MathUtils.clamp(centerDist / (available * 0.5), -1, 1);
    }

    this.poolParams.stepEdgeIndex = best.edge.index;
    this.poolParams.stepOffset = offsetNorm;
    this.applyStepLayoutFromParams();
  }
  ...
  setupStepDragging() {
    ...
    dom.addEventListener("pointerdown", (event) => {
      if (!this.poolGroup || this.customizeMode || this.poolEditor?.isDragging) return;
      const step = this._pickStepAtPointer(event);
      if (!step) return;

      this.captureUndoState("Move steps");
      this.selectedStep = step;
      this.updateHighlightForStep(step, true);
      this.clearHoverHighlight();
      this.stepDrag.active = true;
      this.stepDrag.pointerId = event.pointerId;
      this.stepDrag.step = step;
      this.stepDrag.pendingEvent = event;
    ...
    dom.addEventListener("pointermove", (event) => {
      if (!this.stepDrag.active || event.pointerId !== this.stepDrag.pointerId) return;
      this.stepDrag.pendingEvent = event;
      if (this.stepDrag.raf) return;
      this.stepDrag.raf = requestAnimationFrame(() => {
        this.stepDrag.raf = 0;
        if (this.stepDrag.pendingEvent) this._updateStepPlacementFromPointer(this.stepDrag.pendingEvent);
      });
    });

    const endDrag = (event) => {
      if (!this.stepDrag.active) return;
      if (event && this.stepDrag.pointerId != null && event.pointerId !== this.stepDrag.pointerId) return;
      if (this.stepDrag.raf) cancelAnimationFrame(this.stepDrag.raf);
      this.stepDrag.raf = 0;
      if (this.stepDrag.pendingEvent) this._updateStepPlacementFromPointer(this.stepDrag.pendingEvent);
      this.stepDrag.active = false;
      this.stepDrag.pointerId = null;
      this.stepDrag.step = null;
      this.stepDrag.pendingEvent = null;
...
setupPoolEditor() {
  ...
  setupPoolSliders() {
    ...
    const stepLayoutSelect = document.getElementById("stepLayout");
    if (stepLayoutSelect) {
      stepLayoutSelect.addEventListener("change", (e) => {
        this.captureUndoState("Step layout");
        this.poolParams.stepLayout = e.target.value || "full";
        if (!Number.isFinite(Number(this.poolParams.stepOffset))) {
          this.poolParams.stepOffset = this._getDefaultStepLayoutOffset(this.poolParams.stepLayout);
        }
        this.applyStepLayoutFromParams();
      });
    }
...
// RIPPLE
  ...
  syncSlidersFromParams() {
    ...
    const stepLayoutSelect = document.getElementById("stepLayout");
    if (stepLayoutSelect) {
      stepLayoutSelect.value = this.poolParams.stepLayout || "full";
    }
  }
