...
function generateMeterUVsForCircularDiscGeometry(geo, tileSize) {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    const y = pos.getY(i);
    const z = pos.getZ(i);
    // After rotateX(Math.PI ) the disc's plan lives on XY and thickness on Z.
...
function createCircularDiscGeometry(radius, height) {
  ...
  geo.rotateX(Math.PI ); geo.computeVertexNormals(); return geo;
}
function createWaterSurfaceGeometry(spaShape, xSize, ySize, segments = 96) {
  if (spaShape === 'circular') {
    return new THREE.CircleGeometry(Math.max(0.01, xSize ), Math.max(24, segments));
  }
...
function updateSpaSeats(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    if (seats.ring) {
      ...
      const wallInnerRadius = Math.max(0.12, radius - SPA_WALL_THICKNESS );
...
function updateSpaWalls(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    const waterHeight = Math.max(0.01, (SPA_TOP_OFFSET + SPA_WATER_TOP_FINE_ADJUST) - SPA_WATER_BOTTOM_WORLD); const waterLen = l + 1.0 * (t + overflow); const waterWid = w + 1.0 * (t + overflow); water.geometry.dispose(); water.geometry = new THREE.BoxGeometry(waterLen, waterWid, waterHeight); water.position.set(0,0,SPA_WATER_BOTTOM_WORLD + waterHeight * 0.5 - spa.position.z); water.rotation.set(0,0,0);
  }
...
export function snapToPool(spa) {
  ...
  if (close === "left") {
    ...
    const snapToOuter = !disableExternalSnap && Math.abs(x - outerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
  ...
  if (close === "front") {
    ...
    const snapToOuter = !disableExternalSnap && Math.abs(y - outerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
...
export function createSpa(poolParams, scene, options = {}) {
  ...
  const walls = {
    left: new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), wallMat.clone()),
    right: new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), wallMat.clone()),
    front: new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), wallMat.clone()),
    back: new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), wallMat.clone())
  };
  ...
  walls.ring = new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), wallMat.clone());
  ...
  const seats = {
    front: new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), seatMat.clone()),
    back: new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), seatMat.clone()),
    left: new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), seatMat.clone()),
    right: new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), seatMat.clone())
  };
  ...
  seats.ring = new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 1), seatMat.clone());
  ...
  const floor = new THREE.Mesh(new THREE.BoxGeometry(1, 3.0, 0.1), floorMat);
...
const water = createPoolWater(new THREE.BoxGeometry(1, 3.0, 1));
