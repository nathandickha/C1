...
function updateSpaSeats(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    if (seats.ring) {
      ...
      generateMeterUVsForCircularRingGeometry(seats.ring.geometry, tileSize);
...
function updateSpaWalls(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    if (walls.ring){ walls.ring.geometry.dispose(); walls.ring.geometry=createCircularRingGeometry(radius+t*0.5,Math.max(0.05,radius-t*0.5),h); generateMeterUVsForCircularRingGeometry(walls.ring.geometry, tileSize); walls.ring.position.set(0,0,-h/2); walls.ring.visible=true; }
    const waterRadius = Math.max(0.05, radius + 0.5 * (t + overflow)); water.geometry.dispose(); water.geometry = createWaterSurfaceGeometry('circular', waterRadius * 2, waterRadius * 2, 96); water.scale.set(1,1,1); water.position.set(0,0,(SPA_TOP_OFFSET + SPA_WATER_TOP_FINE_ADJUST) - spa.position.z); water.rotation.set(0,0,0);
  } else {
    ...
    const waterLen = l + 1.0 * (t + overflow); const waterWid = w + 1.0 * (t + overflow); water.geometry.dispose(); water.geometry = createWaterSurfaceGeometry('square', waterLen, waterWid, 96); water.scale.set(1,1,1); water.position.set(0,0,(SPA_TOP_OFFSET + SPA_WATER_TOP_FINE_ADJUST) - spa.position.z); water.rotation.set(0,0,0);
  }
...
const water = createPoolWater(new THREE.PlaneGeometry(1, 1, 64, 64));
