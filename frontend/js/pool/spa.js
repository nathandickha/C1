...
function updateSpaWalls(spa) {
  ...
  if (spaShape === 'circular') {
    ...
    const waterHeight = Math.max(0.01, (SPA_TOP_OFFSET + SPA_WATER_TOP_FINE_ADJUST) - SPA_WATER_BOTTOM_WORLD); const waterRadius = Math.max(0.05, radius + 0.5 * (t + overflow)); water.geometry.dispose(); water.geometry = new THREE.BoxGeometry(waterRadius * 2, waterRadius * 2, waterHeight); water.position.set(0,0,SPA_WATER_BOTTOM_WORLD + waterHeight * 0.5 - spa.position.z); water.rotation.set(0,0,0);
  } else {
